...
1. Nếu có `docs/STATUS.md`, đọc trước; trong hồ sơ dự án chỉ nạp `Current inputs`, rồi reference đúng `Next action`. Khi chuyển phase, phải đưa mọi record bắt buộc của next action vào `Current inputs`; thiếu record bắt buộc là routing error cần sửa trước write. Nếu chưa có starter/STATUS, suy phase từ yêu cầu và chỉ đọc reference cần cho tác vụ.
