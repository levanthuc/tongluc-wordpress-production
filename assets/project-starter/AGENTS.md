...
- Đọc `docs/STATUS.md` trước, sau đó chỉ đọc `Current inputs` và reference đúng `Next action`. Thiếu record bắt buộc của phase trong `Current inputs` là routing error phải sửa trước write. Hồ sơ `Approved` là nguồn quyết định; thực hiện next action rồi dừng ở cổng duyệt kế tiếp.
